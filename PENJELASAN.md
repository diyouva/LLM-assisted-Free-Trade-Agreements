...
| **Mode 1 sampai 4** | Empat cara perdagangan jasa internasional (lintas batas, konsumsi luar negeri, kehadiran komersial, pergerakan orang) |
...
| 4 | **Trade in Services** | Komitmen liberalisasi jasa Mode 1 sampai 4 |
...
| Kategori | Status | Entropy | Penjelasan |
|---------|--------|---------|-----------|
| Customs Procedures | **Konvergen** | 1.00 | Ketiga perjanjian membagi proporsinya hampir sama, sejalan dengan baseline WTO Trade Facilitation |
| General Provisions | Cukup konvergen | 0.91 | Definisi dan pengecualian umum mengikuti template ASEAN yang mirip |
| Dispute Settlement | **Fragmentasi** | 0.47 | AHKFTA tidak punya bab sendiri (mengacu ke WTO DSU); RCEP dan AANZFTA punya mekanisme independen |
| Intellectual Property | **Fragmentasi** | 0.37 | AHKFTA tidak punya bab IP, RCEP paling banyak provisions IP (12) |
| Rules of Origin | (caveat) | 0.97 | Skor entropy tinggi tapi sebagian dipengaruhi few-shot bias; CC vs CTH tetap perbedaan desain nyata |
| Investment | (caveat) | 0.96 | Skor tinggi tapi 4 provisions AHKFTA kemungkinan misklasifikasi; AHKFTA sebenarnya tidak punya bab investasi |
...
└──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘
        ...
        │  AHKFTA: 8.9%   │    │  RQ3 ✓ Customs konvergen    │
        └─────────────────┘    └──────────────────────────┘
...
**Cara baca:**
- Bar **rose (smoky rose)** = run terbaik (highlight)
- Bar **biru muda** = run menengah
- Bar **taupe** = run terbawah
- **LLaMA Zero-Shot** mencatat akurasi tertinggi (0.480)
- **Qwen Chain-of-Thought** mencatat Macro-F1 tertinggi (0.442)
- **LLaMA Chain-of-Thought** justru terbawah, CoT merugikan LLaMA
...
- **Bar biru** (entropy ratio mendekati 1.00) = **konvergen** (ketiga perjanjian mirip di kategori ini)
- **Bar rose** (entropy ratio rendah) = **fragmentasi** (distribusi sangat tidak merata)
- **Bar taupe** (di tengah) = konvergensi sedang

**Baca dari atas ke bawah:**
- Atas (skor tinggi) = kategori yang sudah mulai punya template regional seragam, contohnya Customs Procedures (1.00)
- Bawah (skor rendah) = kategori yang tiap perjanjian masih punya desain sendiri-sendiri, contohnya Intellectual Property (0.37)
...
### Lampiran: Figures (output Python)
...
**RQ3 - Konvergensi atau fragmentasi?**
**Keduanya, tapi tidak seimbang.** Customs Procedures adalah satu-satunya kategori yang benar-benar konvergen (entropy 1.00), sejalan dengan baseline WTO Trade Facilitation yang dipakai ketiga pihak. Dispute Settlement (0.47) dan Intellectual Property (0.37) adalah yang paling terfragmentasi, terutama karena AHKFTA tidak punya bab tersebut sama sekali. Topik prosedural lebih harmonis daripada topik substantif, tapi sebagian besar fragmentasi yang terlihat adalah perbedaan cakupan, bukan perbedaan aturan.

### Batas Kemampuan Sistem Ini

| Keterbatasan | Artinya |
|-------------|---------|
| Akurasi 32 sampai 48% | Cocok hanya untuk penyaringan awal; harus diverifikasi ke sumber asli untuk keputusan penting |
...
*Dokumen ini ditulis dalam Bahasa Indonesia sebagai panduan memahami proyek. Untuk detail teknis lengkap, lihat `README.md`, `REPORT.md`, dan `JOURNAL_PAPER.md`.*
