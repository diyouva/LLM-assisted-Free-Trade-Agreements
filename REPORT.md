...
| **AHKFTA** | ASEAN + Hong Kong, China | 2019 | Main text + Annexes 2 to 11 |
| **AANZFTA** | ASEAN + Australia + NZ | 2010 (amended 2014) | Main text, First Protocol, Implementing Arrangement, Tariff Understanding |

Seven PDFs were processed. AHKFTA is partially scanned, requiring OCR. After extraction, the corpus contains **4,059 provisions**, the smallest meaningful legal unit (an article, paragraph, or structured annex entry), filtered to 80 to 1,500 characters.
...
## 3. Methodology and What the Project Learned Along the Way
...
One observation worth highlighting for policy: **RCEP and AHKFTA both use a 40% Regional Value Content threshold for rules of origin**, the ASEAN standard. But they choose different Change in Tariff Classification rules: RCEP uses change at the heading level (CTH), while AHKFTA requires a change at the chapter level (CC). A chapter-level change requires more substantial transformation of the goods. On paper, the same goods can satisfy RCEP's CTH rule and fail AHKFTA's CC rule even with identical 40% RVC compliance, the kind of detail an analyst would want flagged before a closer reading.
...
- **Most convergent:** Customs Procedures (entropy ratio 1.00). All three agreements allocate roughly equal shares of their text to this chapter, consistent with the WTO Trade Facilitation baseline that all three parties follow.
- **Most fragmented:** Intellectual Property (0.37) and Dispute Settlement (0.47). Both scores are driven mainly by AHKFTA having zero provisions in those categories, not by RCEP and AANZFTA writing those chapters in incompatible ways. The fragmentation is a structural scope difference, not a drafting conflict.
- **Caveat on Rules of Origin and Investment.** Both score high on the entropy ratio but their convergence is partly artefactual. Rules of Origin is inflated by the few-shot prompt's two goods-trade examples; Investment is inflated by 4 AHKFTA provisions classified as investment-adjacent even though AHKFTA has no dedicated Investment chapter.
...
### 5.2 What Did Not Work and Why It Matters
...
- **In under an hour:** Extract all provisions from a new FTA PDF, embed them, and run the classification to get a category distribution. This replaces two to three days of manual triage.
- **With current validation in the 32 to 48% accuracy range:** treat every individual label as a triage hint, not an answer. The value is prioritisation and aggregation, not autonomous legal judgement.
...
This project demonstrates that an LLM-based pipeline can extract, classify, and comparatively analyse FTA provisions, producing a structured first-pass triage layer for analysts comparing agreements. Three key findings stand out:

**Model and prompt engineering decisions matter more than expected, and interact with model architecture in opposing ways.** Chain-of-thought prompting raised Qwen's Macro-F1 by 1.8 points (from 0.424 to 0.442, the best result in the study) but lowered LLaMA's by 10.4 points (from 0.431 to 0.327, the worst result). Few-shot examples degraded both models relative to zero-shot, with LLaMA falling 9.5 points and Qwen falling 5.1 points. This cross-model asymmetry in CoT sensitivity was only discoverable through systematic validation, not from model documentation or casual testing.
...
**Rules of Origin and Dispute Settlement reveal the deepest structural divergences.** AHKFTA allocates nearly half its sampled provisions to Rules of Origin (48%) while dedicating none to Dispute Settlement or Intellectual Property. These are scope differences that reflect different negotiating mandates: AHKFTA as a goods-access instrument, RCEP and AANZFTA as broader integration frameworks. Customs Procedures is the only category where the three agreements are genuinely in sync, while Dispute Settlement and Intellectual Property sit at the fragmented end mainly because AHKFTA does not include those chapters. Procedural topics look more harmonised than substantive ones, but most apparent fragmentation comes from missing chapters rather than opposing rules.

The pipeline is fully reproducible, uses only free-tier APIs, and can be pointed at any new FTA PDF. For the best Macro-F1 the recommended configuration is Qwen 3 32B with chain-of-thought; for raw accuracy and cross-model consistency, LLaMA 3.3 70B with zero-shot is the alternative.
...
├── REPORT.md           this document (project report)
